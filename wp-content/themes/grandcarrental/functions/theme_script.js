...
{	
	...
	jQuery("#content_builder_sort li a.ppb_duplicate").on( 'click', function(){
		...
		//Display edit button
	    var editURL = tgAjax.ajaxurl+'?action=grandcarrental_ppb&ppb_post_type='+postType+'&shortcode='+myCheckId+'&rel='+randomId+'&width=800&height=900&page_id='+jQuery('#ppb_page_id').val();
...
jQuery(document).ready(function(){
	...
	jQuery("#ppb_module_wrapper li").on( 'click', function(){
	    ...
	    if(myCheckId != '' && targetType == 'module')
	    {
			...
			//Display edit button
	    	var editURL = tgAjax.ajaxurl+'?action=grandcarrental_ppb&ppb_post_type='+postType+'&shortcode='+myCheckId+'&rel='+randomId+'&width=800&height=900&page_id='+jQuery('#ppb_page_id').val()+'&builder_action=add';
	    	builderItem+= '<a data-rel="'+randomId+'" href="'+editURL+'" class="ppb_edit"><i class="fa fa-edit"></i></a>';
	...
	jQuery('#import_demo_content li').on( 'click', function(){
	    ...
	    var selectedDemo = jQuery(this).data('demo');
	    jQuery('#grandcarrental_import_demo_content').val(selectedDemo);
	});
	...
	jQuery('.pp_import_content_button').on( 'click', function(){
        ...
        var data = {
            'action': 'grandcarrental_import_demo_content',
            'demo': selectDemo
        };
	...
	//Custom functions for handle car options box
	var postType = jQuery('#car_header_type').val();
	switch(postType) 
	{
	    case 'Vimeo Video':
	        jQuery('#post_option_car_header_vimeo').show();
	        jQuery('#post_option_car_header_youtube').hide();
	    break;
	    
	    case 'Youtube Video':
	        jQuery('#post_option_car_header_youtube').show();
	        jQuery('#post_option_car_header_vimeo').hide();
	    break;
	...
	jQuery('#car_header_type').on( 'change', function(){
	...
	jQuery('#get_styling_color_content li').on( 'click', function(){
        ...
        var data = {
            'action': 'grandcarrental_get_styling',
