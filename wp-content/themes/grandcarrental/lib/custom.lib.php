...
**/
function grandcarrental_pagination($pages = '', $range = 4)
{
	 $showitems = ($range * 2)+1;
	 
	 $paged = grandcarrental_get_paged();
...
function grandcarrental_get_review($post_id = '', $rating_field = '')
{
	$rating_arr = array();
...
*	Setup comment style
**/
function grandcarrental_comment($comment, $args, $depth) 
{
		...
		<div class="gravatar">
      	...
      	<div class="right <?php if($has_avatar == '') { ?>fullwidth<?php } ?>">
			<?php if ($comment->comment_approved == '0') : ?>
         		<em><?php echo esc_html_e('(Your comment is awaiting moderation.)', 'grandcarrental') ?></em>
         		<br />
      		<?php endif; ?>
			...
			<div class="comment_date"><?php echo date_i18n(GRANDCARRENTAL_THEMEDATEFORMAT, strtotime($comment->comment_date)); ?> <?php echo esc_html_e('at', 'grandcarrental') ?> <?php echo date_i18n(GRANDCARRENTAL_THEMETIMEFORMAT, strtotime($comment->comment_date)); ?></div>
			<?php 
      			if($depth < 3)
      			{
      		?>
      			<?php comment_reply_link(array_merge( $args, array('depth' => $depth,
'reply_text' =>  __('Reply', 'grandcarrental'), 'login_text' => __('Login to Reply', 'grandcarrental'), 'max_depth' => $args['max_depth']))) ?>
			<?php
				}
			?>
...
function grandcarrental_substr($str, $length, $minword = 3)
{
    $sub = '';
    $len = 0;
...
**/
function grandcarrental_posts($sort = 'recent', $items = 3, $echo = TRUE, $show_thumb = TRUE) 
{
	$return_html = '';
	...
	if($sort == 'recent')
	{
		...
		$title = esc_html__('Recent Posts', 'grandcarrental');
	}
	...
	if(!empty($posts))
	{
		...
		$count_post = count($posts);
			...
			foreach($posts as $post)
			{
				...
				$return_html.= '<a href="'.get_permalink($post->ID).'">'.$post->post_title.'</a><div class="post_attribute">'.date_i18n(GRANDCARRENTAL_THEMEDATEFORMAT, get_the_time('U', $post->ID)).'</div>';
				$return_html.= '</li>';
...
function grandcarrental_cat_posts($cat_id = '', $items = 5, $echo = TRUE, $show_thumb = TRUE) 
{
	$return_html = '';
	...
	if(!empty($posts))
	{
		...
		$return_html.= '">';
			...
			foreach($posts as $key => $post)
			{
				...
				$return_html.= '<a href="'.get_permalink($post->ID).'">'.grandcarrental_substr($post->post_title, 50).'</a><div class="post_attribute">'.date_i18n(GRANDCARRENTAL_THEMEDATEFORMAT, get_the_time('U', $post->ID)).'</div>';
				$return_html.= '</li>';
			}	
...
function grandcarrental_image_from_description($data) {
    preg_match_all('/<img src="([^"]*)"([^>]*)>/i', $data, $matches);
    return $matches[1][0];
}


function grandcarrental_select_image($img, $size) {
    $img = explode('/', $img);
...
function grandcarrental_get_flickr($settings) 
{
	...
	$flickr_cache_path = grandcarrental_THEMEUPLOAD.'/flickr_'.$settings['id'].'_'.$settings['items'].'.cache';
	...
	if(!file_exists($flickr_cache_path) OR $flickr_cache_timer > 15)
	{
		...
		foreach ($feed->get_items() as $key => $item)
		{
			...
			$img = grandcarrental_image_from_description($item->get_description()); 
			$thumb_url = grandcarrental_select_image($img, 1);
			$large_url = grandcarrental_select_image($img, 4);
...
function grandcarrental_get_instagram($username, $access_token, $items = 8)
{   
	$wp_filesystem = grandcarrental_get_wp_filesystem();
    ...
    {
	    $instagram_cache_path = grandcarrental_THEMEUPLOAD.'/instagram_'.$username.'_'.$items.'.cache';
...
function grandcarrental_get_browser() 
{ 
...
function grandcarrental_auto_link_twitter($tweet)
{
...
function grandcarrental_resort_gallery_img($all_photo_arr)
{
...
function grandcarrental_apply_content($pp_content) {
...
function grandcarrental_apply_builder($page_id, $post_type = 'page', $print = TRUE) 
{
	...
	if(isset($ppb_form_item_arr[0]) && !empty($ppb_form_item_arr[0]))
	{
	    ...
	    foreach($ppb_form_item_arr as $key => $ppb_form_item)
	    {
	    	...
	    	if($print)
	    	{
	    		echo grandcarrental_apply_content($ppb_shortcode_code);
	    	}
	    	else
	    	{
		    	$ppb_page_content.= grandcarrental_apply_content($ppb_shortcode_code);
	    	}
        }
    }
...
function grandcarrental_get_excerpt_by_id($post_id)
{
...
function grandcarrental_get_image_id($url) 
{
	$attachment_id = attachment_url_to_postid($url);
	return $attachment_id;
}

function grandcarrental_aasort(&$array, $key) 
{
    $sorter=array();
    $ret=array();
    reset($array);
...
function grandcarrental_update_urls($options,$oldurl,$newurl)
{	
	$wpdb = grandcarrental_get_wpdb();
	$results = array();
	$queries = array(
	'content' =>		array("UPDATE $wpdb->posts SET post_content = replace(post_content, %s, %s)",  esc_html__('Content Items (Posts, Pages, Custom Post Types, Revisions)','grandcarrental') ),
	'excerpts' =>		array("UPDATE $wpdb->posts SET post_excerpt = replace(post_excerpt, %s, %s)", esc_html__('Excerpts','grandcarrental') ),
	'attachments' =>	array("UPDATE $wpdb->posts SET guid = replace(guid, %s, %s) WHERE post_type = 'attachment'",  esc_html__('Attachments','grandcarrental') ),
	'links' =>			array("UPDATE $wpdb->links SET link_url = replace(link_url, %s, %s)", esc_html__('Links','grandcarrental') ),
	'custom' =>			array("UPDATE $wpdb->postmeta SET meta_value = replace(meta_value, %s, %s)",  esc_html__('Custom Fields','grandcarrental') ),
	'guids' =>			array("UPDATE $wpdb->posts SET guid = replace(guid, %s, %s)",  esc_html__('GUIDs','grandcarrental') )
	);
	    ...
	    if( $option == 'custom' ){
	    	...
	    	for( $page = 0; $page < $pages; $page++ ) {
	    		...
	    		if( trim($value) == '' )
	    			...
	    			$edited = grandcarrental_unserialize_replace( $oldurl, $newurl, $value );
...
function grandcarrental_unserialize_replace( $from = '', $to = '', $data = '', $serialised = false ) 
{
    try {
    	if ( is_string( $data ) && ( $unserialized = @unserialize( $data ) ) !== false ) {
    		$data = grandcarrental_unserialize_replace( $from, $to, $unserialized, true );
    	}
    	elseif ( is_array( $data ) ) {
    		$_tmp = array( );
    		foreach ( $data as $key => $value ) {
    			$_tmp[ $key ] = grandcarrental_unserialize_replace( $from, $to, $value, false );
    		}
    		$data = $_tmp;
    		unset( $_tmp );
    	}
...
function grandcarrental_get_first_title_word($title) {
	return $title;
}

function grandcarrental_menu_layout() {
...
/**
* grandcarrental_is_woocommerce_page - Returns true if on a page which uses WooCommerce templates (cart and checkout are standard pages with shortcodes and which are also included)
*
* @access public
* @return bool
*/
function grandcarrental_is_woocommerce_page() 
{
...
function grandcarrental_check_system()
{
	$has_error = 0;
...
function grandcarrental_hex_darker($hex,$factor = 30)
{
    $new_hex = '';
...
function grandcarrental_get_dynamic_sidebar($index = 1)
{
    $sidebar_contents = "";
    ob_start();
    dynamic_sidebar($index);
...
function grandcarrental_hex_to_rgb($hex) 
{
	$hex = str_replace("#", "", $hex);
	$color = array();
...
function grandcarrental_available_widgets() 
{
	$wp_registered_widget_controls = grandcarrental_get_registered_widget_controls();
...
function grandcarrental_import_data( $data ) 
{
	$wp_registered_sidebars = grandcarrental_get_registered_sidebars();

	// Have valid data?
	// If no data or could not decode
	if ( empty( $data ) || ! is_object( $data ) ) {
		wp_die(
			esc_html__('Import data could not be read. Please try a different file.', 'grandcarrental' ),
			'',
	...
	$available_widgets = grandcarrental_available_widgets();
	...
	foreach ( $data as $sidebar_id => $widgets ) {
		...
		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
			...
			$sidebar_message = esc_html__('Sidebar does not exist in theme (using Inactive)', 'grandcarrental' );
		}
		...
		foreach ( $widgets as $widget_instance_id => $widget ) {
			...
			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
				...
				$widget_message = esc_html__('Site does not support widget', 'grandcarrental' ); // explain why widget not imported
			}
			...
			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
				...
				foreach ( $single_widget_instances as $check_id => $check_widget ) {
					...
					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
						...
						$widget_message = esc_html__('Widget already exists', 'grandcarrental' ); // explain why widget not imported
			...
			if ( ! $fail ) {
				...
				if ( $sidebar_available ) {
					...
					$widget_message = esc_html__('Imported', 'grandcarrental' );
				} else {
					$widget_message_type = 'warning';
					$widget_message = esc_html__('Imported to Inactive', 'grandcarrental' );
				}
			...
			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__('No Title', 'grandcarrental' ); // show "No Title" if widget instance is untitled
...
function grandcarrental_count_client_images($client_id = '')
{
	if(!empty($client_id))
	{
		$images_counter = 0;
...
function grandcarrental_set_map_api()
{
	...
	{
		wp_enqueue_script("google_maps", "https://maps.googleapis.com/maps/api/js", false, GRANDCARRENTAL_THEMEVERSION, true);
	}
	else
	{
		wp_enqueue_script("google_maps", "https://maps.googleapis.com/maps/api/js?key=".$pp_googlemap_api_key, false, GRANDCARRENTAL_THEMEVERSION, true);
	}
}

function grandcarrental_live_builder_begin_wrapper($item_id = '', $item_type = '')
{
	...
	if(isset($_GET['ppb_live']) && !empty($item_id))
	{
		...
		{
			$return_html.= '<a href="javascript:;" class="ppb_add_after" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Add Content After This', 'grandcarrental' ).'"><i class="fa fa-plus"></i></a>';
		
			$return_html.= '<a href="javascript:;" class="ppb_edit" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Edit', 'grandcarrental' ).'"><i class="fa fa-edit"></i></a>';
			
			$return_html.= '<a href="javascript:;" class="ppb_duplicate" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Duplicate', 'grandcarrental' ).'"><i class="fa fa-copy"></i></a>';
		}
		
		$return_html.= '<a href="javascript:;" class="ppb_remove" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Remove', 'grandcarrental' ).'"><i class="fa fa-trash"></i></a>';
...
function grandcarrental_live_builder_end_wrapper($item_id = '')
{
	//Check if live content builder mode
	if(isset($_GET['ppb_live']) && !empty($item_id))
	{
...
function grandcarrental_get_post_view($post_id = '', $raw = false)
{
...
function grandcarrental_get_post_share($post_id = '', $raw = false)
{
	if($raw)
	{
...
function grandcarrental_get_initial_car_query()
{
	$query_string = '&orderby=post_date&order=DESC&post_type=car&suppress_filters=0&post_status=publish';
...
function grandcarrental_get_demo_url($path = '')
{
	$demo_url = 'http://themes.themegoods.com/grandcarrental/demo'.$path;
	return $demo_url;
}

function grandcarrental_set_recently_view_cars()
{
	$tg_post = grandcarrental_get_wp_post();
	
	if(!isset($_COOKIE['grandcar_recently_view_cars'])) 
	{
		$recently_view_cars = array();
		$recently_view_cars[] = $tg_post->ID;
		
		setcookie('grandcar_recently_view_cars', json_encode($recently_view_cars), strtotime( '+2 days' ), '/');
	}
	else
	{
		$recently_view_cars = json_decode($_COOKIE['grandcar_recently_view_cars']);
		
		if(!in_array($tg_post->ID, $recently_view_cars))
		{
			$recently_view_cars[] = $tg_post->ID;
		}
...
function grandcarrental_get_recently_view_cars()
{
	$recently_view_cars = array();
	...
	return array_reverse($recently_view_cars);
}

function grandcarrental_themegoods_action() 
{
...
function grandcarrental_starts_with($haystack, $needle) 
{
