...
<?php
	...
	if(isset($_GET['ppb_live']))
	{
		...
		wp_enqueue_script("grandcarrental-live-builder", get_template_directory_uri()."/js/custom_livebuilder.js", false, GRANDCARRENTAL_THEMEVERSION, true);
	}

	$ppb_form_data_order = get_transient('grandcarrental_'.$post->ID.'_data_order');
	$ppb_page_content = '';
	...
	{
	    $ppb_shortcode_code = '';
	    $ppb_form_item_data = get_transient('grandcarrental_'.$post->ID.'_data');
	    $ppb_form_item_size = get_transient('grandcarrental_'.$post->ID.'_size');
